...
for date in range(0,n_day):
    for stock in range(0,n_stock):
        ...
        y[i] = (np.std(close_return.iloc[start+date+200:start+date+210,stock]) \
        -np.std(close_return.iloc[start+date+200-10:start+date+200,stock]))*np.sqrt(250/10)
        x[i,:,5] = y[i]
        i = i+1
